...
const ScrollExpand = ({
  ...
  // Keep all props in a ref so the RAF loop always reads fresh values
  const propsRef = useRef({
    startWidth, startHeight, startRadius, endRadius,
    mediaZoom, scrollDistance, holdDistance, smoothing,
    overlayScrim, useWindowScroll, enabled, mediaOpacity,
  });
  propsRef.current = {
    startWidth, startHeight, startRadius, endRadius,
    mediaZoom, scrollDistance, holdDistance, smoothing,
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      stage.style.setProperty('--se-title-size', `${clamp(w * 0.055, 18, 64)}px`);
    };
